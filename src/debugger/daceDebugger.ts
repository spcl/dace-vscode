...
implements vscode.DebugConfigurationProvider {
    ...
    resolveDebugConfiguration(
		...
		if (!config.type && !config.request && !config.name) {
			let msg = "Please make sure you have a launch.json file with a configuration of the type 'dace-debug' to use this debugger";
    ...
    async provideDebugConfigurations(folder?: vscode.WorkspaceFolder, token?: vscode.CancellationToken): Promise<vscode.DebugConfiguration[]> {
        ...
        const items: MenuItem[] = [
			{ label: "DaCe Debugger", description: "Default", type: configType.DEFAULT},
			{ label: "DaCe Debugger", description: "Manual", type: configType.MANUAL}
		];
