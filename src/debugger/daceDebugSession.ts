...
export class DaceDebugSession extends LoggingDebugSession {
    ...
    protected launchRequest(
         ...
         *   in the launch.json file and pass it to the attribute 'entirePythonConfig'
         */
        let entirePythonConfig;
        if(!args.pythonConfig || args.pythonConfig === 'default'){
            ...
            if(!args.pythonLaunchName){
                let msg = "Please make sure to define 'pythonLaunchName' for dace-debug in your launch.json file or set 'pythonConfig' to default";
                ...
                entirePythonConfig = getConfig(args.pythonLaunchName, this.folder);
                if(!entirePythonConfig){
                    let message = "Please make sure you have a configurations with the name '" + args.pythonLaunchName + "' in your launch.json file.";
                    return vscode.window.showErrorMessage(message).then(_ => {
                        this.sendEvent(new TerminatedEvent());	
                        return; // abort launch
                    });
                ...
                /* * 
                 *  Deppending on if the user set an environment variable or not 
                 *  we either add it to the variables or create an 'env' attribute
                 * */ 
                if(entirePythonConfig.env){
                    entirePythonConfig.env.DACE_compiler_build_type = buildType;
                } else{
                    entirePythonConfig.env = {
                        "DACE_compiler_build_type": buildType
                    };
                }
            }
        ...
        /**
          * Default: 
          *   We detect the operating system and set 'cppConfig: default (win/gdb) Attach'
          *   in the 'Python C++ Debugger configuration'
          * Manual:
          *   Otherwise, the user specifies the the configuration manually by passing 
          *   the name of the configuration to cppAttachName. We then pass the name
          *   to the 'Python C++ Debugger' for it to handle
          */
        let cppAttribute;
        let cppValue;
        if(!args.cppConfig || args.cppConfig === 'default'){
            ...
            if(!args.cppAttachName){
                let msg = "Please make sure to define 'cppAttachName' for dace-debug in your launch.json file or set 'cppConfig' to default";
                return vscode.window.showInformationMessage(msg).then(_ => {
                    this.sendEvent(new TerminatedEvent());	
                    return; // abort launch
                });
