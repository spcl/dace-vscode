...
export class BreakpointHandler extends vscode.Disposable {
    ...
    private getNode(line: number, path: fs.PathLike): Node | undefined {
        ...
        let mapPy = this.jsonFromPath(path);
        if (!mapPy) {
            return undefined;
        }
    ...
    private getCppLine(node: Node, path: fs.PathLike) {
        let mapCpp = this.jsonFromPath(path);
        let states = mapCpp[node.sdfg_id];
        if (!states) return undefined
        let nodes = states[node.state_id];
        if (!nodes) return undefined;
        let cppLine = nodes[node.node_id];
        if (!cppLine) return undefined
        return cppLine;
    }

    private jsonFromPath(path: fs.PathLike) {
        /**
