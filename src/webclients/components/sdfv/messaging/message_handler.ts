...
import { CorrectnessOverlay, sdfgEditContinueShow, sdfgEditDifferenceShow } from '../correctness/correctness';
    ...
    public handleMessage(message: any): void {
        ...
        switch (message.type) {
            ...
            case 'correctness_report':
                if (renderer) {
                    const olManager = renderer.get_overlay_manager();
                    renderer.get_overlay_manager()?.register_overlay(
                        CorrectnessOverlay
                    );
                    const ol = olManager?.get_overlay(CorrectnessOverlay);
                    if (ol && ol instanceof CorrectnessOverlay) {
                        ol.newReports(message.reports);
                        if (message.diffText !== undefined &&
                            message.diffText !== null)
                            ol.setDiffThreshold(parseFloat(message.diffText));
                        ol.setDiffRange(message.diffRange);
                        ol.setDiffText(message.diffText);
